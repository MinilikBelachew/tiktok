...
const initiateDeposit = async (req, res) => {
    try {
        ...
        const chapaResponse = await axios.post("https://api.chapa.co/v1/transaction/initialize", chapaRequest, {
            headers: {
...
const requestWithdrawal = async (req, res) => {
    try {
        const { amount, type = "MOBILE_MONEY", // Default to mobile money
        accountName, accountNumber, bankName, } = req.body;
        const userId = req.user?.id;
        if (!userId) {
...
const processWithdrawal = async (withdrawalId) => {
    try {
        ...
        const response = await axios.post("https://api.chapa.co/v1/transfers", transferData, { headers });
...
export { getWithdrawalHistory, requestWithdrawal, getPaymentHistory, handleWebhook, initiateDeposit, checkPaymentStatus, paymentSuccess, processWithdrawal, };
