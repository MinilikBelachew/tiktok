...
const getBetsByUserId = async (req, res) => {
...
const getBetsMyMarketId = async (req, res) => {
...
const getTotalBet = async (req, res) => {
    const marketId = parseInt(req.params.marketId, 10);
    try {
        const bets = await prisma.bet.findMany({
            where: { marketId },
...
const resolveMarket = async (req, res) => {
    ...
    try {
        ...
        const totalAmount = market.bets.reduce((sum, bet) => sum.plus(bet.amount), new Decimal(0));
...
export { createBet, getBetsByUserId, getBetById, updateBet, deleteBet, resolveMarket, getBetsMyMarketId, getAllBets, getTotalBet, getTotalUser };
