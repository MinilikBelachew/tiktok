...
const WalletPage: React.FC = () => {
  const [activeMode, setActiveMode] = useState<Mode>('deposit');
  const [amount, setAmount] = useState('');
  const [selectedMethod, setSelectedMethod] = useState<string>('');

  const handleContinue = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Wallet submit:', { activeMode, amount, selectedMethod });
  };
  ...
  return (
    ...
    <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {/* Left panel */}
            <div className="bg-white border border-gray-300 rounded-xl p-4 sm:p-6 min-w-0">
              {/* Mode toggle */}
              <div className="flex gap-3 mb-6">
                <button
                  onClick={() => setActiveMode('deposit')}
                  className={`flex-1 rounded-2xl py-4 text-sm sm:text-base font-semibold ${
                    activeMode === 'deposit' ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-700'
                  }`}
                >
                  Deposit
                </button>
                <button
                  ...
                  className={`flex-1 rounded-2xl py-2 text-sm sm:text-base font-semibold ${
                    activeMode === 'withdraw' ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700'
                  }`}
                >
                  Withdraw
                </button>
                ...
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {Array.from({ length: 8 }).map((_, idx) => (
                      <tr key={idx}>
                        <td className="px-3 py-2 border border-gray-300 text-green-600 font-semibold">{idx === 0 ? '+546' : ''}</td>
